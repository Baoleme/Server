...
CREATE TABLE IF NOT EXISTS `baoleme`.`Restaurant` (
  ...
  `logo_url` VARCHAR(255) NOT NULL DEFAULT 'http://ocphk5wc7.bkt.clouddn.com/default-logo.png',
...
CREATE TABLE IF NOT EXISTS `baoleme`.`Dish` (
  ...
  CONSTRAINT `fk_Dish_Category1`
    ...
    REFERENCES `baoleme`.`Category` (`category_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `baoleme`.`QRcode`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `baoleme`.`QRcode` (
  `qrcode_id` INT NOT NULL AUTO_INCREMENT,
  `restaurant_id` INT NOT NULL,
  `table` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`qrcode_id`, `restaurant_id`),
